...
/**
 * @author ：enilu
 * @since  ：Created in 2019/7/30 23:07
 */
        ...
        if (isLoginAttempt(request, response)) {
            try {
                if(executeLogin(request, response)){
                    if (userService.tokenIsAllowed(user,oldToken)){
                        return true;
                    } else {
                ...
                if(e.getMessage().contains("expired")){
                    ...
                    if(userService.refreshTokenIsValid(oldToken)) {
                        ...
                        userService.kickOutAll(user);
                        userService.addIn(user,newToken);
                        return true;
                    }
                }
                response401(request, response);
...
//    }
    ...
    protected boolean preHandle(ServletRequest request, ServletResponse response) throws Exception {
        ...
        String[] whiteList = {"http://10.150.74.140:8001","http://localhost:8001","http://127.0.0.1:8001"};
        ...
        httpServletResponse.setHeader("Access-control-Allow-Origin", isValid ? myOrigin : "null");
