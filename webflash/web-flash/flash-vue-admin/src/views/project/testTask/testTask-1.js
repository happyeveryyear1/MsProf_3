import { getAllTaskList, removeTask, getTaskList, addTask, editTask, executeTask, saveResult, getJobResult } from '@/api/project/testTask'
...
export default {
  ...
  methods: {
    init() {
      ...
      // getAllActivityList({testactivityID:this.params_testactivityId}).then(response => { // 通过测试活动名查测试活动id
      //   this.activityID = this.params_testactivityId
      //   this.params_testactivityName = response.data[0].testactivityName
      //   this.projectId= response.data[0].projectName
      ...
      this.$confirm('是否提交表单内容?', '提示', {
        ...
        this.$refs['form'].validate((valid) => {
          if (valid) {
            ...
            //   let listAlltask = response.data
            //   let taskNums = response.data.length
              this.checkCase = []
              ...
              // var j = 0
              // while (j !== taskNums && listAlltask[j].taskName !== this.form.taskName) {
              //   j++
              // }
              // if (j < taskNums && this.form.id !== listAlltask[j].id) {
              //   this.$notify.error({
              //     title: '错误',
              //     message: '该任务名已存在，表单已自动生成新的任务名，请重新提交！'
              //   })
              //   let newTaskNum = Number(this.form.taskName.split('-')[2])+1
              ...
              // } else {
                ...
                if(this.checkCase[0] || this.cantEditCase){
                  if(this.form.testConfiguration.length>256){
                    this.$notify.error({
                      title: '错误',
                      message: '测试机配置信息过多'
                    })
                    ...
                    if(this.form.artifactInfos.length>65535){
                      this.$notify.error({
                        title: '错误',
                        message: '组件信息过多'
                      })
                          ...
                          // } else { // 添加任务-保存用例
                            ...
                            while (this.checkCase[i]) {
                              ...
                              addCase({
                                ...
                                j++
                                if(j===this.checkCase.length){ // 如果用例都保存完了，就在质量模块保存任务
                                  newTask({projectName: this.projectId,verTag: this.form.taskName}).then(response => {
