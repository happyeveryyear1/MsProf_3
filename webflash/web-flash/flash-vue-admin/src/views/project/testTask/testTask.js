...
import { getAllTaskList, removeTask, getTaskList, addTask, editTask, executeTask, saveResult, getJobResult, mvcDiagnoserAnalyzeHar, mvcDiagnoserAnalyzeSwagger } from '@/api/project/testTask'
import { getAllActivityList, getActivityList, saveTaskNum } from '@/api/project/testActivity'
import { getAllCaseList, importAllTestCase, selectTestCase, collectInfo, importArtifacts, deleteSelect, removeCase, addCase, editCase, pageInfo, pageNumber } from '@/api/project/testCase'
...
import { performanceResult, performanceAnalysis, harAnalysis, functionAnalysis, functionResult } from '@/api/project/testCase'
...
export default {
    ...
    props: ['projectlist'],
    data() {
        const validateOldVersion = (rule, value, callback) => {
            if (value) {
                if (value === this.form.versionNum) {
                    callback(new Error('旧版本号与版本号相同'))
                } else {
                    callback()
                }
            } else {
        ...
        return {
            ...
            form: {
                ...
                id: '', // 任务id（自动生成）
                ipAddress: '192.168.17.22', // 测试机配置IP地址
                browser: 'Chrome', // 测试机配置浏览器
                ...
                harStatus: '-1', // har分析状态
                harAnalyzeStatus: '-1',
                swaggerAnalyzeStatus: '-1', // swagger分析状态
                funcStatus: '-1', // 功能缺陷分析状态
                baseUrl: ''
            },
            search_taskName: '', // 与查询条件中的任务名绑定
            listQuery: {
                page: 1,
                limit: 10,
                beginTime: undefined,
            ...
            params_testactivityId: '', // 测试活动页面传过来的“测试活动id”
            params_testactivityName: '', // 测试活动页面传过来的“测试活动id”对应的测试活动名
            total: 0, // 列表总数
            dataNum: 0, // 记录任务总数目
            ...
            exeBtn: false, // 展开行中的执行按钮
            applicationName: '', // 应用名（性能接口所需）
            id: 1, // 树节点
            taskCaseList: [], // 任务已经保存的测试用例列表
            functionResultTitle: '', // 功能缺陷分析dialog名称
            functionResultVisible: false, // 功能缺陷分析dialog可视状态
            isAnalyzeHaring: false, // Har分析时按钮可用状态
            isAnalyzeSwaggering: false, // Swagger分析时按钮可用状态

            formRules: {
                versionNum: [
                    { required: true, message: '版本号不能为空', trigger: 'blur' },
                    { pattern: /^[a-zA-Z0-9_\.]+$/, message: '只支持英文、数字和小数点' },
                    { min: 2, max: 10, message: '长度在2-10个字符间', trigger: 'blur' }
                ],
                ipAddress: [
                ...
                oldVersion: [
                    { pattern: /^[a-zA-Z0-9_\.]+$/, message: '只支持英文、数字和小数点' },
                    { trigger: 'blur', validator: validateOldVersion },
                    { min: 2, max: 10, message: '长度在2-10个字符间', trigger: 'blur' }
                ],
                directory: [
                ...
                artifactInfoName: [
                    { max: 15, message: '长度不大于15', trigger: 'blur' }
                ],
                artifactInfoVersion: [
                    { max: 8, message: '长度不大于8', trigger: 'blur' }
                ],
                baseUrl: [
    ...
    methods: {
        init() {
            ...
            this.params_testactivityId = this.$route.params.testactivityName
            this.listQuery.testactivityName = this.params_testactivityId
            if (this.$route.query.taskName) {
                this.search_taskName = this.$route.query.taskName
            }
            this.listLoading = true
            getActivityList({ page: 1, limit: 10, id: this.params_testactivityId }).then(response => { // 通过测试活动名查测试活动id
                this.activityID = this.params_testactivityId
                this.params_testactivityName = response.data.records[0].testactivityName
                this.projectName = response.data.records[0].proName
                this.projectId = response.data.records[0].projectName
                this.tasks = response.data.records[0].tasks
                this.fetchData()
                getAllProList({ id: this.projectId }).then(response => { // 通过项目id查项目名和测试用例git地址
        ...
        analyzeHar(taskName) {
            ...
            this.isAnalyzeHaring = true
            mvcDiagnoserAnalyzeHar({ 'taskName': taskName }).then(response => {
                // 分析结束
                console.log('mvcDiagnoserAnalyzeHar response: ', response)
                this.isAnalyzeHaring = false
                this.fetchData()
            }).catch(err => {
                this.$notify.error({
                    title: '错误',
        ...
        // 分析Swagger
        analyzeSwagger(taskName) {
            console.log('taskName: ', taskName)
            this.isAnalyzeSwaggering = true
            mvcDiagnoserAnalyzeSwagger({ 'taskName': taskName }).then(response => {
                // 分析结束
                console.log('mvcDiagnoserAnalyzeSwagger response: ', response)
                this.isAnalyzeSwaggering = false
                this.fetchData()
            }).catch(err => {
                this.$notify.error({
                    title: '错误',
        ...
        fetchData() {
            ...
            getAllTaskList().then(response => { // 获取所有任务数
        ...
        add() { // 添加任务
            ...
            if (this.list[0]) { // 自动获取任务名
                const taskNum = Number(this.list[0].taskName.split('-')[2]) + 1
        ...
        save() { // 保存任务和用例
            this.$confirm('是否提交表单内容?', '提示', {
                ...
                this.$refs['form'].validate((valid) => {
                    if (valid) {
                        // getAllTaskList({testactivityName:this.params_testactivityId}).then(response => { // 获取测试活动下的所有任务数，方便检测是否重名
                        addTask({
                            ...
                            tester: this.$store.state.user.profile.id,
                            id: this.form.id,
                            ...
                            harStatus: this.form.harStatus,
                            harAnalyzeStatus: this.form.harAnalyzeStatus,
                            swaggerAnalyzeStatus: this.form.swaggerAnalyzeStatus,
                            funcStatus: this.form.funcStatus,
                            ...
                            if (err.msg === '任务名已被使用，请重新输入') {
                                this.$notify.error({
                                    title: '错误',
                                    message: '该任务名已存在，表单已自动生成新的任务名，请重新提交！'
                                })
                                const newTaskNum = Number(this.form.taskName.split('-')[2]) + 1
                                this.form.taskName = 'task-' + this.activityID + '-' + newTaskNum
                            } else {
                                this.$notify.error({
                                    title: '错误',
                                    message: '测试活动保存出错，请重试！'
                                })
                            }
                            this.submitBtn = false
                        })
        ...
        edit() { // 编辑任务和用例
            if (this.checkSel()) {
                ...
                if (this.selRow.exeStatus === '-1') { // 编辑未执行的任务
                    ...
                    getAllCaseList({ taskName: this.selRow.id }).then(response => { // 获取任务原有用例
                        ...
                        if (this.taskCaseList.length !== 0) {
                            importAllTestCase({ dir: this.selRow.directory, git: this.gitAddress }).then(response => {
                                ...
                                this.treeDataArray = []
                                this.forTreeData(treeData_all)
                                this.taskCaseList.forEach(element => {
                                    const oneCase = {}
                                    oneCase.caseId = element.id
                                    oneCase.id = ''
                                    oneCase.key = element.menuModule + '/' + element.testcaseName
                                    oneCase.label = element.testcaseName
                                    if (this.treeDataArray.length === 0) { // 表示该用例不在当前的git仓库中，要多开一个树节点
                                        this.testList.push(oneCase.key) // 把该用例加入树结构中
                                        treeData_all = this.pathToTree(this.testList)
                                        oneCase.id = this.id - 1
                                        this.checkIds.push(oneCase.id)
                                        this.caseListArray.push(oneCase)
                                    } else {
                                        for (let i = 0; i < this.treeDataArray.length; i++) {
                                            ...
                                            if (i === this.treeDataArray.length - 1) { // 表示该用例不在当前的git仓库中，要多开一个树节点
                                                this.testList.push(oneCase.key) // 把该用例加入树结构中
                                                treeData_all = this.pathToTree(this.testList)
                                                oneCase.id = this.id - 1
                                                this.checkIds.push(oneCase.id)
                    ...
                    getAllCaseList({ taskName: this.selRow.id }).then(response => {
                        ...
                        for (let i = 0; i < allCases.length; i++) {
                            const menu = allCases[i].menuModule
                ...
                const selData = JSON.parse(JSON.stringify(this.selRow))
                this.form = selData
                this.form.tester = selData.testerName
        ...
        //                     pdata.tester = this.$store.state.user.profile.id // 创建人要改一下
        //                     pdata.id = ''
        //                     pdata.testResult = ''
        ...
        //                     pdata.harStatus = '-1',
        //                     pdata.harAnalyzeStatus = '-1',
        //                     pdata.swaggerAnalyzeStatus = '-1',
        //                     pdata.funcStatus = '-1',
        //                     addTask(pdata).then(response => {
        ...
        //                                         i++
        //                                         if (i === cases.length) {
        //                                             newTask({ projectName: this.projectId, verTag: pdata.taskName }).then(response => {
        //                                                 // this.tasks = Number(this.tasks) + 1
        //                                                 // saveTaskNum({id:this.activityID,num: this.tasks})
        //                                                 this.fetchData()
        //                                             }).catch(err => {
        ...
        //                         })
        //                     }).catch(err => {
        //                         if (err.response.data.message === 'Subject does not have permission [tasksAdd]') {
        //                             this.$notify.error({
        //                                 title: '错误',
        //                                 message: '没有创建任务权限'
        //                             })
            ...
            if (this.form.oldVersion) {
                ...
                importArtifacts(pdata).then(response => {
                    if (response.data.msg === '成功') {
                        const artifactsInfo = response.data.data.artifacts
                        ...
                        // 这块到时候要删除
                        if (!this.form.artifactInfoName && !this.form.artifactInfoVersion) {
                            this.$notify.error({
                                title: '错误',
                                message: '该版本组件信息为空'
                            })
                        ...
                        this.$notify.error({
                            title: '错误',
                            message: '该项目还未有任务保存过组件信息，导入失败。请手动填写'
                        })
                    }
                }).catch(err => {
                    this.$notify.error({
                        title: '错误',
                        message: '导入组件信息异常，请重试'
                    })
                })
            } else {
                this.$notify.error({
                ...
                pageNumber(pdata).then(response => {
                    ...
                    pageInfo(pdata).then(response => {
                        ...
                        if (this.pageNum === this.pageInfosNum) { // 若sitespeed分析完成，则可以评估
                            ...
                            queryData['proj_id'] = this.projectId
        ...
        getItem() { // 取出数据
            ...
            const checkItem = JSON.parse(window.sessionStorage.getItem('checkCase') || '[]')
            const ids = []
        ...
        getPerformanceResult(taskName) { // 展开行的性能分析-执行完成时点击获取分析结果
            this.mapData = []
            const pdata = {}
            pdata.projectName = this.projectName
            ...
            performanceResult(pdata).then(response => {
                ...
                this.performanceResultTitle = taskName + '：性能瓶颈分析结果'
        ...
        analyse(taskName, testcases, index) { // 性能分析-执行
            if (this.list[index].exeStatus === '1') {
                if (Number(testcases) >= 5) {
                    getAllCaseList({ taskName: this.list[index].id }).then(response => { // 获取该任务下的测试用例
                        let harNum = Number(testcases)
                        for (let i = 0; i < response.data.length; i++) {
                            if (response.data[i].harPath === '') {
                                harNum--
                            }
                        }
                        if (harNum >= 5) {
                            ...
                            performanceAnalysis({ projectName: this.projectName, taskId: taskName, applicationName: this.applicationName }).catch(err => {
                            ...
                            this.$alert('该任务测试用例生成的har包数目少于5个，性能分析终止！', '提示', {
                                confirmButtonText: '确定'
                            })
                        }
                    })
        ...
        refresh(taskName, index) { // 性能分析-刷新
            this.$set(this.isRefresh_analyse, taskName, true)
            const pdata = {}
            pdata.projectName = this.projectName
            pdata.taskId = taskName
            performanceResult(pdata).then(response => {
                ...
                this.$notify.error({
                    title: '错误',
                    message: '刷新状态获取异常，请重试'
                })
            })
        },

        getTaskResult(taskName, index) { // 获取任务执行结果-刷新
            this.$set(this.isRefresh_task, taskName, true)
            getAllCaseList({ taskName: this.list[index].id }).then(response => { // 获取该任务所有测试用例
                ...
                if (percentage === 100) {
                    ...
                    saveResult({ taskName: taskName, executionTime: exeTime, executionResult: success + ',' + failure, exeStatus: '1', harStatus: '1' })
                    const pdata = {}
                    pdata.projectName = this.projectName
                    pdata.taskId = taskName
                    this.list[index].harStatus = '1'
                    // disable har analysis to avoid 502 alert
                    // harAnalysis(pdata).catch(err => { // 500异常即代表接口有问题，没进行har分析，若是其他异常，应该就是超时，已经在进行分析
                    //   if(err.response.status === 500){
                    //     // disable har analysis err info
                    //     console.log("任务进行har分析异常，请展开任务重新进行har分析")
                    //     // this.$notify.error({
                    //     //   title: '错误',
                    //     //   message: '任务进行har分析异常，请展开任务重新进行har分析'
                    //     // })
                    //     this.list[index].harStatus = '-1'
                    //     saveResult({taskName: taskName, harStatus: '-1'})
                    //   }
                    // })
        ...
        reHar(taskName, index) { // 重新进行har分析
            const pdata = {}
            pdata.projectName = this.projectName
            pdata.taskId = taskName
            this.list[index].harStatus = '1'
        ...
        funcAnalyse(taskName, index) { // 功能缺陷分析-执行
            if (this.list[index].exeStatus === '1') {
                if (this.list[index].testResult.split(',')[1] !== '0') {
                    this.list[index].funcStatus = '0'
                    saveResult({ taskName: taskName, funcStatus: '0' })
                    functionAnalysis({ projectName: this.projectName, taskId: taskName, applicationName: this.applicationName }).then(response => {
                        this.list[index].funcStatus = '1'
                    ...
                    this.$alert('该任务没有执行失败的用例，所以不能进行功能缺陷分析！', '提示', {
                        confirmButtonText: '确定'
                    })
                }
            } else {
                this.$alert('该任务还未执行完成，不能进行功能分析！', '提示', {
                    confirmButtonText: '确定'
                })
            }
        },

        getFuncResult(taskName) { // 展开行的性能分析-执行完成时点击获取分析结果
            this.mapData = []
            const pdata = {}
            pdata.projectName = this.projectName
            pdata.taskId = taskName
            functionResult(pdata).then(response => {
                ...
                this.functionResultTitle = taskName + '：功能缺陷分析结果'
                this.functionResultVisible = true
            }).catch(err => {
                this.$notify.error({
                    title: '错误',
                    message: '结果获取异常，请重试'
