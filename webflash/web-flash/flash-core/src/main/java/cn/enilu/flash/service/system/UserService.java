...
//        cacheDao.hset(CacheDao.SESSION,account,user);
    ...
    public void addIn(User user,String token){
        try {
            cacheDao.hset(CacheDao.SESSION,user.getAccount(),token);
        } catch (Exception e){
            e.printStackTrace();
        }
    }
    public boolean kickOutAll(User user){
        try {
            String token = (String) cacheDao.hget(CacheDao.SESSION, user.getAccount());
    ...
    public boolean tokenIsAllowed(User user, String token) {
        try{
            String allowed = (String) cacheDao.hget(CacheDao.SESSION,user.getAccount());
            return allowed.equals(token);
        }catch (Exception e){
            e.printStackTrace();
            return false;
        }
