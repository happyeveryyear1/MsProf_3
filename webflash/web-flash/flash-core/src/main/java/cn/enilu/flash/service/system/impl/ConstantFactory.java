...
import cn.enilu.flash.dao.project.*;
...
public class ConstantFactory implements IConstantFactory {
    ...
    private projectListRepository projectListRepository = SpringContextHolder.getBean(projectListRepository.class);
...
//        return "--";
    ...
    public String getRoleName(String roleIds) {
        int length = roleIds.split(",").length;
        String id;
        String val = new String();
        for(int i=0;i<length;i++){
            id = roleIds.split(",")[i];
            Optional<Role> optional = roleRepository.findById(Long.parseLong(id));
            if(i==length-1){
                val += optional.get().getName();
            }else {
                val += optional.get().getName() + ",";
            }
        }
     ...
     */
    @Override
    public String getProNameById(Long proId){
        projectList projectList = getProject(proId);
        if (projectList != null) {
            String val = projectList.getProjectName();
            return val;
        }
        return "--";
    }

    private projectList getProject(Long id) {
        Optional<projectList> optionalProjectList = projectListRepository.findById(id);
