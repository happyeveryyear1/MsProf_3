...
import com.navercorp.pinpoint.plugin.tomcat.interceptor.*;
...
public class TomcatPlugin implements ProfilerPlugin, TransformTemplateAware {
    ...
    public static ConcurrentHashMap<String, String> taskQueueRecord = new ConcurrentHashMap();
    ...
    public void addTransformers(TomcatConfig config) {
        // Add server metadata
        addStandardService();   // Service组件的实现类，startInternal方法，记录ServerInfo
        addTomcatConnector();   // Connector组件实现类，initInternal方法，记录协议、端口
        addWebappLoader();      // 记录webapploader加载的jar包

        // Add async listener. Servlet 3.0
        addRequest();           // 异步Servlet监控
        // Hide pinpoint headers & Trace HTTP response status code
        addRequestFacade(config);
        // Entry Point
        addStandardHostValve(); // 监控方法
        
        // SocketProcessor监控，监控socket被处理的时间点
        addSocketProcessorBase();
    }
    ...
    private void addSocketProcessorBase() {
        // transformTemplate.transform("org.apache.tomcat.util.net.SocketProcessorBase", SocketProcessorBaseTransform.class);
        transformTemplate.transform("org.apache.tomcat.util.net.NioEndpoint$SocketProcessor", NioSocketProcessorBaseTransform.class);
        transformTemplate.transform("org.apache.tomcat.util.net.Nio2Endpoint$SocketProcessor", Nio2SocketProcessorBaseTransform.class);
        transformTemplate.transform("org.apache.tomcat.util.threads.TaskQueue", TaskQueueTransform.class);
    }
    
    public static class SocketProcessorBaseTransform implements TransformCallback {
        
        @Override
        public byte[] doInTransform(Instrumentor instrumentor, ClassLoader classLoader, String className, Class<?> classBeingRedefined, ProtectionDomain protectionDomain, byte[] classfileBuffer) throws InstrumentException {
            final InstrumentClass target = instrumentor.getInstrumentClass(classLoader, className, classfileBuffer);
            // Remove bind trace
            final InstrumentMethod method = target.getDeclaredMethod("doRun");
            if (method != null) {
                method.addInterceptor(SocketProcessInterceptor.class);
            }
            return target.toBytecode();
        }
    }
    
    public static class NioSocketProcessorBaseTransform implements TransformCallback {
        ...
        public byte[] doInTransform(Instrumentor instrumentor, ClassLoader classLoader, String className, Class<?> classBeingRedefined, ProtectionDomain protectionDomain, byte[] classfileBuffer) throws InstrumentException {
            ...
            final InstrumentMethod method = target.getDeclaredMethod("doRun");
            if (method != null) {
                method.addInterceptor(NioSocketProcessorInterceptor.class);
            }
            return target.toBytecode();
        }
    }
    
    public static class TaskQueueTransform implements TransformCallback {
        
        @Override
        public byte[] doInTransform(Instrumentor instrumentor, ClassLoader classLoader, String className, Class<?> classBeingRedefined, ProtectionDomain protectionDomain, byte[] classfileBuffer) throws InstrumentException {
            final InstrumentClass target = instrumentor.getInstrumentClass(classLoader, className, classfileBuffer);
            // Remove bind trace
            final InstrumentMethod method = target.getDeclaredMethod("offer", "java.lang.Runnable");
            if (method != null) {
                method.addInterceptor(TaskQueueInterceptor.class);
            }
            return target.toBytecode();
        }
    }
    
    public static class Nio2SocketProcessorBaseTransform implements TransformCallback {
        ...
        public byte[] doInTransform(Instrumentor instrumentor, ClassLoader classLoader, String className, Class<?> classBeingRedefined, ProtectionDomain protectionDomain, byte[] classfileBuffer) throws InstrumentException {
            ...
            final InstrumentMethod method = target.getDeclaredMethod("doRun");
            if (method != null) {
                method.addInterceptor(Nio2SocketProcessorInterceptor.class);
            }
            return target.toBytecode();
        }
    }
