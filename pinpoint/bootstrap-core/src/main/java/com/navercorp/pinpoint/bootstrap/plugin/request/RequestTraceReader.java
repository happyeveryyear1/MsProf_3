...
public class RequestTraceReader<T> {
    ...
    public Trace read(T request){
        Trace trace = doRead(request);
        // read Task_ID和Test_ID
        String taskId = getHeader(request, Header.HTTP_TASK_ID.toString());
        String testTd = getHeader(request, Header.HTTP_TEST_ID.toString());
        String myToken = getHeader(request, Header.HTTP_MY_TOKEN.toString());
        String toolType = getHeader(request, Header.HTTP_TOOL_TYPE.toString());
        // restore teskId and testId to Trace Annotation
        trace.getSpanRecorder().recordAttribute(AnnotationKey.HTTP_TEST_ID, testTd);
        trace.getSpanRecorder().recordAttribute(AnnotationKey.HTTP_TASK_ID, taskId);
        trace.getSpanRecorder().recordAttribute(AnnotationKey.HTTP_MY_TOKEN, myToken);
        trace.getSpanRecorder().recordAttribute(AnnotationKey.HTTP_TOOL_TYPE, toolType);
        HeaderContext.set(taskId, testTd, myToken, toolType);
        return trace;
    }
    
    // Read the transaction information from the request.
    public Trace doRead(T request) {
    ...
    private String getHeader(T request, String name){
        String origin = requestAdaptor.getHeader(request, name);
        if(origin == null)
            return null;
        try {
            return URLDecoder.decode(origin, "utf8");
        }catch (UnsupportedEncodingException e){
            logger.warn("[*] Decode {} Error: {}", name, e);
            logger.warn("[*] While decode header {} ", request.toString());
        }
        return origin;
    }
