...
public class DefaultRequestTraceWriter<T> implements RequestTraceWriter<T> {
    ...
    private void setMsProf(T header){
        String taskId = HeaderContext.getTaskId();
        if(taskId != null){
            clientHeaderAdaptor.setHeader(header, Header.HTTP_TASK_ID.toString(), taskId);
        }
        String testId = HeaderContext.getTestId();
        if(testId != null) {
            clientHeaderAdaptor.setHeader(header, Header.HTTP_TEST_ID.toString(), testId);
        }
        String myToken = HeaderContext.getMyToken();
        if(testId != null) {
            clientHeaderAdaptor.setHeader(header, Header.HTTP_MY_TOKEN.toString(), myToken);
        }
        String msProfFlag = HeaderContext.getToolType();
        if(testId != null) {
            clientHeaderAdaptor.setHeader(header, Header.HTTP_TOOL_TYPE.toString(), msProfFlag);
        }
    }
}